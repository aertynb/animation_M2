...
typedef struct
{
  ...
  /*-------------------------------------------------------------------*/
  /* champs pour récupérer les infos 'contextuelles'                   */
...
{
  /* vidange de la table : libération de tout ce qui traine */
...
{
  GFL_MTENV.tabsize = 1024;              // taille de départ
...
{
  /* table pleine : traceur désactivé */
  if (GFL_MTENV.full  ==true) return false;
...
{
  if (GFL_MTENV.full) return false;
...
{
  /* activation auto au 1° appel                                  */
  /* si l'initialisation échoue : comportement normal de <free> */
  if (GFL_MTENV.full) return;
  
  if (gfl_mtauto_start()==false) return free(ptr);
