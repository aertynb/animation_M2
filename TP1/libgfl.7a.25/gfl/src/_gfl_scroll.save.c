...
static void gfl_DrawScrollh(GFLscroll *scroll)
{
  ...
  if (scroll->factor!=1.)
  {
    glRasterPos2i((18),(scroll->ycurs-4));
         if (scroll->factor<10000. && scroll->factor>1.00) sprintf(str,"(x%.0f)",scroll->factor);
    else if (scroll->factor<    1. && scroll->factor>0.01) sprintf(str,"(x%.1f)",scroll->factor);
    else sprintf(str,"(x%.e)",scroll->factor);
    glutBitmapString(GLUT_BITMAP_HELVETICA_10,(const uchar*)str);
  }
    /* curseur */
  glRasterPos2i((scroll->xcurs+scroll->w+2),(scroll->ycurs-4));
       if (scroll->dprm) sprintf(str,"%.1lf",*scroll->dprm*scroll->factor);
  else if (scroll->iprm) 
        { if (scroll->factor!=1.) sprintf(str,"%.1lf",*scroll->iprm*scroll->factor);
          else                    sprintf(str,"%d"   ,*scroll->iprm); 
        }
  else                            sprintf(str,"%.2lf",scroll->min + scroll->cursor*(scroll->max-scroll->min));
  glutBitmapString(GLUT_BITMAP_HELVETICA_10,(const uchar*)str);
}
...
static __inline__ void gfl_DrawScrollv(GFLscroll *scroll)
{
  ...
  if (scroll->factor!=1.)
  {
    glRasterPos2d((scroll->xcurs-10),(Yscrollstart-32));
         if (scroll->factor<1000. && scroll->factor>1.00) sprintf(str,"%.0f",scroll->factor);
    else if (scroll->factor<   1. && scroll->factor>0.01) sprintf(str,"%.1f",scroll->factor);
    else  sprintf(str,"%.e",scroll->factor);
    glutBitmapString(GLUT_BITMAP_HELVETICA_10,(const uchar*)str);
  }
  /* curseur */
       if (scroll->dprm) sprintf(str,"%.1lf",*scroll->dprm*scroll->factor);
  ...
  else                            sprintf(str,"%.1lf",scroll->min + scroll->cursor*(scroll->max-scroll->min));
