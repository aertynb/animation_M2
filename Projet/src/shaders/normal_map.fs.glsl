...
void main()
{
  ...
  vec4 emissiveFromTexture =
      texture(uEmissiveTexture, vTexCoords);
  vec4 occlusionFromTexture =
      texture(uOcclusionTexture, vTexCoords);
  vec3 normalFromTexture = texture(uNormalTexture, vTexCoords).rgb * 2.0 - 1.0;

  // vec3 Nbis = normalize(vTBN * normalFromTexture);
  N = normalize(vTBN * normalFromTexture);
  ...
  float NdotL = clamp(dot(N, L), 0.01, 1.);
  // float NdotV = clamp(dot(Nbis, V), 0., 1.);
  ...
  vec3 emissive = SRGBtoLINEAR(emissiveFromTexture).rgb * uEmissiveFactor * 1.5;
