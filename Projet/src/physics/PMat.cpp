...
{
  _vit += h * _frc /
          _m; // intégration 1 : vitesse m.F(n) = (V(n+1)-V(n))/h -EXplicite
  _pos +=
      h * _vit; // intégration 2 : position V(n+1) = (X(n+1)-X(n))/h -IMplicite
...
// simple échange des 2 premières lignes par rapport à Leapfrog -> ça change
// tout
void PMat::update_euler_exp(float h)
{
  _pos +=
      h * _vit; // intégration 1 : position V(n) = (X(n+1)-X(n-1))/h -EXplicite
  _vit += h * _frc /
          _m; // intégration 2 : vitesse m.F(n) = (V(n+1)-V(n))/h -EXplicite
  _frc = glm::vec3(0.f); // on vide le buffer de force
}
